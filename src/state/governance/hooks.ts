...
export function useUniContract() {
  ...
  const uniAddress = useMemo(() => (chainId ? DOUGH[chainId]?.address : undefined), [chainId])
...
export function useAllProposalData(): { data: ProposalData[]; loading: boolean } {
  ...
  const uni = useMemo(() => (chainId ? DOUGH[chainId] : undefined), [chainId])
...
export function useUserVotes(): { loading: boolean; votes: CurrencyAmount<Token> | undefined } {
  ...
  return useMemo(() => {
    const uni = chainId ? DOUGH[chainId] : undefined
...
export function useDelegateCallback(): (delegatee: string | undefined) => undefined | Promise<string> {
  ...
  return useCallback(
    (delegatee: string | undefined) => {
      ...
      if (!uniContract) throw new Error('No DOUGH Contract!')
